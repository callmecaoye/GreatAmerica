...
public class AuthenticationManager {
    ...
    public static synchronized AuthenticationManager getInstance(Context context) {
        if (INSTANCE == null) {
            ...
            editor.putString("oidc_clientId", OIDCConstants.CLIENT_ID);
            editor.putString("oidc_redirectUrl", OIDCConstants.REDIRECT_URI);
            editor.putString("oidc_scopes", OIDCConstants.SCOPES);
                        ...
                        if (!futureManager.isCancelled()) {
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    try {
                                        authenticationCallback.onSuccess(mOIDCAccountManager.getRefreshToken(account.name, null));
