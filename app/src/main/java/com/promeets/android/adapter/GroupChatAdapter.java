...
import com.promeets.android.custom.CircleProgressBar;
...
import com.promeets.android.util.TxtUtils;
...
public class GroupChatAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static final String APPOINT_CUSTOM_TYPE = "appointment";
    ...
    private static final int VIEW_TYPE_APPOINT_ME = 100;
    private static final int VIEW_TYPE_APPOINT_OTHER = 110;
    ...
    public GroupChatAdapter(Context context) {
        mContext = context;
        width = ((BaseActivity)mContext).getWidth();
    ...
    public void load(String channelUrl) {
        try {
            ...
            File dataFile = new File(appDir, TxtUtils.generateMD5(SendBird.getCurrentUser().getUserId() + channelUrl) + ".data");
    ...
    public void save() {
        try {
            ...
            if (mChannel != null) {
                ...
                String md5 = TxtUtils.generateMD5(data);

                // Save the data into file.
                File appDir = new File(mContext.getCacheDir(), SendBird.getApplicationId());
                appDir.mkdirs();

                File hashFile = new File(appDir, TxtUtils.generateMD5(SendBird.getCurrentUser().getUserId() + mChannel.getUrl()) + ".hash");
                File dataFile = new File(appDir, TxtUtils.generateMD5(SendBird.getCurrentUser().getUserId() + mChannel.getUrl()) + ".data");
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_APPOINT_ME:
                View myAppointView = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.list_item_group_chat_appoint_me, parent, false);
                return new MyAppointHolder(myAppointView);
            case VIEW_TYPE_APPOINT_OTHER:
                View otherAppointView = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.list_item_group_chat_appoint_other, parent, false);
                return new OtherAppointHolder(otherAppointView);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (holder.getItemViewType()) {
            ...
            case VIEW_TYPE_USER_MESSAGE_OTHER:
                ...
                break;
            case VIEW_TYPE_APPOINT_ME:
                ((MyAppointHolder) holder).bind(mContext, (UserMessage) message, mChannel, isContinuous, isNewDay, isTempMessage, isFailedMessage, mItemClickListener, mItemLongClickListener, position);
                break;
            case VIEW_TYPE_APPOINT_OTHER:
                ((OtherAppointHolder) holder).bind(mContext, (UserMessage) message, mChannel, isContinuous, isNewDay, isTempMessage, isFailedMessage, mItemClickListener, mItemLongClickListener, position);
                break;
            case VIEW_TYPE_FILE_MESSAGE_IMAGE_ME:
    ...
    public int getItemViewType(int position) {
        ...
        if (message instanceof UserMessage) {
            ...
            if (userMessage.getSender().getUserId().equals(SendBird.getCurrentUser().getUserId())) {
                if (userMessage.getCustomType().equals(GroupChatAdapter.APPOINT_CUSTOM_TYPE))
                    return VIEW_TYPE_APPOINT_ME;
                else
                    return VIEW_TYPE_USER_MESSAGE_ME;
            } else {
                if (userMessage.getCustomType().equals(GroupChatAdapter.APPOINT_CUSTOM_TYPE))
                    return VIEW_TYPE_APPOINT_OTHER;
                else
    ...
    public class MyUserMessageHolder extends RecyclerView.ViewHolder {
        ...
        void bind(Context context, final UserMessage message, GroupChannel channel, boolean isContinuous, boolean isNewDay, boolean isTempMessage, boolean isFailedMessage, final OnItemClickListener clickListener, final OnItemLongClickListener longClickListener, final int position) {
            ...
            if (isFailedMessage) {
                readReceiptText.setText("Failed");
                readReceiptText.setVisibility(View.VISIBLE);
            } else if (isTempMessage) {
                readReceiptText.setText("Sending");
                readReceiptText.setVisibility(View.VISIBLE);
            } else {
                readReceiptText.setVisibility(View.INVISIBLE);
                // Since setChannel is set slightly after mAdapterNotify is created
                /*if (channel != null) {
            ...
            if (message.getCustomType().equals(URL_PREVIEW_CUSTOM_TYPE)) {
                try {
                    ...
                    Glide.with(context).load(previewInfo.getImageUrl()).into(urlPreviewMainImageView);
    ...
    public class OtherUserMessageHolder extends RecyclerView.ViewHolder {
        TextView messageText, editedText, timeText, readReceiptText, dateText;
        ...
        public OtherUserMessageHolder(View itemView) {
            ...
            editedText = (TextView) itemView.findViewById(R.id.text_group_chat_edited);
            timeText = (TextView) itemView.findViewById(R.id.text_group_chat_time);
            profileImage = (CircleImageView) itemView.findViewById(R.id.image_group_chat_profile);
            readReceiptText = (TextView) itemView.findViewById(R.id.text_group_chat_read_receipt);
            dateText = (TextView) itemView.findViewById(R.id.text_group_chat_date);
        ...
        void bind(Context context, final UserMessage message, GroupChannel channel, boolean isNewDay, boolean isContinuous, final OnItemClickListener clickListener, final OnItemLongClickListener longClickListener, final int position) {
            ...
            if (isContinuous) {
                ...
                Glide.with(context).load(message.getSender().getProfileUrl()).into(profileImage);
            }
    ...
    public class MyAppointHolder extends RecyclerView.ViewHolder {
        ...
        TextView mTxtService, mTxtTime, mTxtLoc;
        View padding;
        Gson gson = new Gson();
        CardView mCard;
        ImageView mIcoLoc;

        MyAppointHolder(View itemView) {
            super(itemView);

            mCard = (CardView) itemView.findViewById(R.id.card_group_chat_message);

            timeText = (TextView) itemView.findViewById(R.id.text_group_chat_time);
            readReceiptText = (TextView) itemView.findViewById(R.id.text_group_chat_read_receipt);
            dateText = (TextView) itemView.findViewById(R.id.text_group_chat_date);

            mTxtService = (TextView) itemView.findViewById(R.id.service_name);
            mTxtTime = (TextView) itemView.findViewById(R.id.time);
            mTxtLoc = (TextView) itemView.findViewById(R.id.location);
            mIcoLoc = (ImageView) itemView.findViewById(R.id.loc_icon);
        ...
        void bind(Context context, final UserMessage message, GroupChannel channel, boolean isContinuous, boolean isNewDay, boolean isTempMessage, boolean isFailedMessage, final OnItemClickListener clickListener, final OnItemLongClickListener longClickListener, final int position) {
            ...
            mTxtService.setText(chatData.serviceTitle);

            EventTimePOJO timePojo = chatData.eventDateList.get(0);
            EventLocationPOJO locPojo = chatData.eventLocationList.get(0);
            mTxtLoc.setText(locPojo.location);
            if (locPojo.status == 2)
                mIcoLoc.setImageResource(R.drawable.ic_chat_call);
            else
                mIcoLoc.setImageResource(R.drawable.ic_chat_loc);
            if (timePojo.utcBeginTime > 0 && timePojo.utcEndTime > 0) {
                DateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
                DateFormat timeFormatter = new SimpleDateFormat("HH:mm");
                Calendar calendar = Calendar.getInstance();
                calendar.setTimeInMillis(timePojo.utcBeginTime);
                String hour1 = timeFormatter.format(calendar.getTime());
                String date = dateFormatter.format(calendar.getTime());
                calendar.setTimeInMillis(timePojo.utcEndTime);
                String hour2 = timeFormatter.format(calendar.getTime());
                mTxtTime.setText(hour1 + " - " + hour2 + " " + date);
            } else {
                DateFormat formatter = new SimpleDateFormat("HH:mm yyyy-MM-dd");
                formatter.setTimeZone(TimeZone.getTimeZone(timePojo.timeZone));
                Date date1 = null;
                Date date2 = null;
                try {
                    date1 = formatter.parse(timePojo.beginHourOfDay + " " + timePojo.detailDay);
                    date2 = formatter.parse(timePojo.endHourOfDay + " " + timePojo.detailDay);
                    //timePojo.utcBeginTime = cal1.getTimeInMillis();
                    //timePojo.utcEndTime = cal2.getTimeInMillis();
                    //Log.d("caoye", timePojo + cal1.getTimeZone().getID());
                    ...
                    //Calendar calendar = Calendar.getInstance();
                    //calendar.setTimeInMillis(timePojo.utcBeginTime);
                    String hour1 = timeFormatter.format(date1);
                    String date = dateFormatter.format(date1);
                    //calendar.setTimeInMillis(timePojo.utcEndTime);
                    String hour2 = timeFormatter.format(date2);
                    mTxtTime.setText(hour1 + " - " + hour2 + " " + date);
    ...
    public class OtherAppointHolder extends RecyclerView.ViewHolder {
        TextView timeText, readReceiptText, dateText;
        TextView mTxtService, mTxtTime, mTxtLoc;
        CircleImageView profileImage;
        CardView mCard;
        ...
        OtherAppointHolder(View itemView) {
            super(itemView);
    ...
    private class MyImageFileMessageHolder extends RecyclerView.ViewHolder {
        ...
        void bind(Context context, final FileMessage message, GroupChannel channel, boolean isNewDay, boolean isTempMessage, boolean isFailedMessage, Uri tempFileMessageUri, final OnItemClickListener listener) {
            ViewGroup.LayoutParams params = fileThumbnailImage.getLayoutParams();
            params.width = width * 2 / 3;
            //params.height = width * 2 / 3;
            fileThumbnailImage.requestLayout();
            ...
            if (isTempMessage && tempFileMessageUri != null) {
                ...
                Glide.with(context).load(tempFileMessageUri.toString()).into(fileThumbnailImage);
            } else {
                ...
                Glide.with(context).load(message.getUrl()).into(fileThumbnailImage);
            }
